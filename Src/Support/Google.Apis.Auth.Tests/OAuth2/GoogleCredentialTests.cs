...
""Unused"": ""unused_value""}}";
        ...
        private IConfiguration BuildConfigurationForCredential(string json) =>
            ...
            // this library.
            new ConfigurationBuilder().AddJsonStream(new MemoryStream(Encoding.UTF8.GetBytes(System.Text.Json.Nodes.JsonNode.Parse(json).ToJsonString()))).Build();
...
""type"": ""service_account""}").ToGoogleCredential();
        ...
        public async Task FromStream_ServiceAccountCredential_GetJwtAccessToken()
        {
            ...
            var header = SystemTextJsonSerializer.Instance.Deserialize<JsonWebSignature.Header>(TokenEncodingHelpers.Base64UrlToString(parts[0]));
            Assert.Equal("JWT", header.Type);
            Assert.Equal("RS256", header.Algorithm);

            var payload = SystemTextJsonSerializer.Instance.Deserialize<JsonWebSignature.Payload>(TokenEncodingHelpers.Base64UrlToString(parts[1]));
